  ...
  try {
    ...
    const Entity17 = axios.get(
      `${tenant}/data/CaseGroups?$format=application/json;odata.metadata=none${
        ...
        userCompany ? `&$filter=dataAreaId eq '${userCompany}'` : ""
      }&$select=GroupId,ProjGroupId,Description&$orderby=Description%20asc`,
      { headers: { Authorization: "Bearer " + token } }
    );
    ...
    const Entity18 = axios.get(
      `${tenant}/data/CaseTypes?$format=application/json;odata.metadata=none${
        ...
        userCompany ? `&$filter=dataAreaId eq '${userCompany}'` : ""
      }&$select=TypeId,Description&$orderby=Description%20asc`,
      { headers: { Authorization: "Bearer " + token } }
    );
    ...
    const Entity26 = axios.get(
      `${tenant}/data/SRF_ProjTables?$format=application/json;odata.metadata=none${
        isTest && numberOfElements ? "&$top=" + numberOfElements : ""
      ...
      .then(
        axios.spread(async (...responses) => {
          ...
          const reply = {
            ...
            NAVTruckEntrances: responses[24].data.value,
            SRF_ProjTables: responses[25].data.value,
            ContractGroups: responses[27].data.value,
            SRF_DeviceCustodians: responses[28].data.value
          };
