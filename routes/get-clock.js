...
router.post("/", async (req, res) => {
  try {
    ...
    const Entity2 = axios.get(
      `${tenant}/data/NAVCaseTimeSheetTrans?$format=application/json;odata.metadata=none${
        isTest && numberOfElements ? "&$top=" + numberOfElements : ""
      }&cross-company=true`,
      { headers: { Authorization: "Bearer " + token } }
    );
    ...
    const Entity7 = axios.get(
      `${tenant}/data/CaseOprTables?$format=application/json;odata.metadata=none${
        isTest && numberOfElements ? "&$top=" + numberOfElements : ""
      }&cross-company=true`,
      { headers: { Authorization: "Bearer " + token } }
    );
      ...
      .then(
        axios.spread(async (...responses) => {
          ...
          const reply = {
            ...
            CaseOprTables: responses[6].data.value,
            SRF_Workers: responses[7].data.value
          };
