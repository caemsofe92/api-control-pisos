...
router.post("/", async (req, res) => {
  try {
    ...
    if (!RecId1 || RecId1.length === 0)
    throw new Error("RecId1 is Mandatory");

    if (!dataAreaId || dataAreaId.length === 0)
    throw new Error("dataAreaId is Mandatory");

    if (!UpdateTareas || UpdateTareas.length === 0)
    throw new Error("UpdateTareas is Mandatory");
